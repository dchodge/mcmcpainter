---
title: "Butterfly MCMC Line Painting Demo"
...
vignette: >
  %\VignetteIndexEntry{Butterfly MCMC Line Painting Demo}
...
# Butterfly MCMC Line Painting Demo

This vignette demonstrates the complete workflow for generating line paintings using the `mcmcPainter` package, featuring the beautiful `butterfly.png` image with 100,000 MCMC iterations for maximum detail and quality.

## 🦋 **Image Analysis**

First, let's analyze the butterfly image to understand its characteristics:
...
library(magick)
butterfly_img <- image_read(image_path)
image_info <- image_info(butterfly_img)
...
Now let's configure the MCMC parameters for optimal performance on the butterfly image:
...
Compare the butterfly results with other images:

```{r performance_comparison, eval=FALSE}
# Performance comparison table
comparison_data <- data.frame(
  Image = c("Butterfly", "Leaf", "Iamami"),
  Dimensions = c(
    paste(config$scaled_width, "x", config$scaled_height),
    "800 x 1422", 
    "788 x 605"
  ),
  Iterations = c(100000, 20000, 100000),
  File_Size_KB = c(
...
### **Why 100K Iterations for Butterfly?**

1. **High Complexity**: The butterfly image has intricate wing patterns and fine details
2. **Large File Size**: 4.2MB indicates rich texture and color variation
3. **Fine Details**: Wings, antennae, and body require precise line placement
4. **Color Gradients**: Smooth transitions need many thin, overlapping strokes

### **Expected Results**

- **Line Count**: 500-1000+ lines for full detail
...
rmarkdown::render("vignettes/butterfly_mcmc_demo.Rmd")

# Option 2: Run directly in R
source("vignettes/butterfly_mcmc_demo.Rmd")

# Option 3: Use the standalone script
source("create_butterfly_triptych.R")
```
...
**Note**: The 100K MCMC run will take 3-5 hours depending on your system. The results will show the full artistic potential of the algorithm on complex, detailed images like the butterfly! 🦋✨
