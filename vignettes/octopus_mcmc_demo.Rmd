---
title: "Octopus MCMC Line Painting Demo"
...
vignette: >
  %\VignetteIndexEntry{Octopus MCMC Line Painting Demo}
...
# Octopus MCMC Line Painting Demo

This vignette demonstrates the complete workflow for generating line paintings using the `mcmcPainter` package, featuring the fascinating `octopus.png` image with 100,000 MCMC iterations for maximum detail and quality.

## 🐙 **Image Analysis**

First, let's analyze the octopus image to understand its characteristics:
...
library(magick)
octopus_img <- image_read(image_path)
image_info <- image_info(octopus_img)
...
Now let's configure the MCMC parameters for optimal performance on the octopus image:
...
Compare the octopus results with other images:

```{r performance_comparison, eval=FALSE}
# Performance comparison table
comparison_data <- data.frame(
  Image = c("Octopus", "Butterfly", "Leaf", "Iamami"),
  Dimensions = c(
  ...
  Iterations = c(100000, 100000, 20000, 100000),
  File_Size_KB = c(
...
### **Why 100K Iterations for Octopus?**

1. **Complex Tentacles**: The octopus has intricate, flowing tentacles that require precise line placement
2. **Texture Details**: Suckers and skin texture need many fine strokes
3. **Color Gradients**: Smooth transitions in the octopus body require overlapping lines
4. **Organic Shapes**: Natural curves and organic forms benefit from high iteration counts

### **Expected Results**

- **Line Count**: 800-1500+ lines for full detail
...
rmarkdown::render("vignettes/octopus_mcmc_demo.Rmd")

# Option 2: Run directly in R
source("vignettes/octopus_mcmc_demo.Rmd")

# Option 3: Use the standalone script
source("create_octopus_triptych.R")
```
...
**Note**: The 100K MCMC run will take 3-5 hours depending on your system. The results will show the full artistic potential of the algorithm on complex, organic images like the octopus! 🐙✨
