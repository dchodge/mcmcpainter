...
#' @docType package
#' @name mcmcArt
#' @useDynLib mcmcArt, .registration = TRUE
...
#' Main MCMC line painting function
...
run_line_painter <- function(image_path,
  ...
  # target is already in [H, W, 3] format from load_image_rgb
  ...
  res <- rjmcmc_line_paint(
    ...
    K_lambda   = 0.5 * width,      # tweak complexity prior by canvas size
    save_every = max(2000, round(iters / 20)),
    out_dir    = out_dir,
    seed       = seed,
    verbose    = TRUE
  )
