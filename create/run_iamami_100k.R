#!/usr/bin/env Rscript
# Run Iamami MCMC with 100,000 Steps
# High-quality run with frequent saving for detailed progress tracking

cat("mcmcPainter: Iamami 100K MCMC Run\n")
cat("==================================\n\n")
...
cat("Expected runtime: 2-4 hours (depending on system)\n\n")

# Run MCMC with high-quality settings
cat("Starting high-quality MCMC run...\n")
...
res <- run_line_painter(
  ...
  out_dir = "inst/results/iamami_100k_high_quality",
  seed = 42,
...
target_img <- load_image_rgb(image_path, 
                            out_w = res$canvas %>% dim() %>% .[2], 
                            out_h = res$canvas %>% dim() %>% .[1])
...
create_triptych(
  ...
  titles = c("Default (White Canvas)", "Best MCMC Result (100K steps)", "True Image"),
  main_title = "Iamami Image: 100K MCMC Progression"
)

# Save triptych in multiple formats
cat("Saving triptych...\n")
output_dir <- "inst/results/iamami_100k_high_quality"

# Save as PDF
pdf_path <- file.path(output_dir, "iamami_100k_triptych.pdf")
save_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
# Save as PNG
png_path <- file.path(output_dir, "iamami_100k_triptych.png")
save_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
cat("\n🎨 High-quality MCMC run completed!\n")
cat("Check the results in:", output_dir, "\n")
cat("The triptych shows the progression from white canvas to final artwork.\n")
cat("With 100K iterations, you should see significant detail and quality improvement!\n")
