#!/usr/bin/env Rscript
# Create Butterfly Triptych with 100K MCMC Steps
# High-quality run for maximum detail and artistic quality

cat("mcmcPainter: Butterfly 100K MCMC Run\n")
cat("====================================\n\n")
...
# Analyze the butterfly image
image_path <- "inst/extdata/butterfly.png"
...
cat("Starting high-quality MCMC run for butterfly...\n")
...
res <- run_line_painter(
  ...
  out_dir = "inst/results/butterfly_100k_high_quality",
  seed = 42,
...
create_triptych(
  ...
  titles = c("Default (White Canvas)", "Best MCMC Result (100K steps)", "True Butterfly Image"),
  main_title = "Butterfly Image: 100K MCMC Progression"
)

# Save triptych in multiple formats
cat("Saving triptych...\n")
output_dir <- "inst/results/butterfly_100k_high_quality"

# Save as PDF
pdf_path <- file.path(output_dir, "butterfly_100k_triptych.pdf")
save_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
# Save as PNG
png_path <- file.path(output_dir, "butterfly_100k_triptych.png")
save_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
cat("\n🦋 High-quality butterfly MCMC run completed!\n")
cat("Check the results in:", output_dir, "\n")
cat("The triptych shows the progression from white canvas to final artwork.\n")
cat("With 100K iterations, you should see incredible detail in the butterfly wings!\n")
