...
# Demonstrates the new dot-based MCMC painter

cat("mcmcPainter: Dot Painting Demo\n")
cat("==============================\n\n")
...
cat("This will create a pointillism-style artwork using dots!\n\n")

res <- run_dot_painter(
  image_path = image_path,
  max_dimension = 800,        # Optimized for dots
  iters = 20000,              # 20K iterations
  out_dir = "inst/results/leaf_dot_painting",
  seed = 42,
...
create_dot_triptych(
  ...
  titles = c("Default (White Canvas)", "Best Dot Painting Result", "True Leaf Image"),
  main_title = "Leaf Image: Dot Painting Progression"
)

# Save triptych in multiple formats
cat("Saving triptych...\n")
output_dir <- "inst/results/leaf_dot_painting"

# Save as PDF
pdf_path <- file.path(output_dir, "leaf_dot_triptych.pdf")
save_dot_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
# Save as PNG
png_path <- file.path(output_dir, "leaf_dot_triptych.png")
save_dot_triptych(
  default_canvas = default_canvas,
  best_canvas = res$best$canvas,
  target_img = target_img,
...
cat("This creates a beautiful pointillism-style painting using thousands of dots!\n")
